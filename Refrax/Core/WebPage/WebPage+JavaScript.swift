...
extension WebPage {
    ...
    ///
    /// `callJavaScript`/`evaluateJavaScript` run scripts inside a forced user
    /// gesture and strip the window's transient activation afterward — erasing
    /// the activation a real user click just granted and breaking
    /// activation-gated APIs the page calls next (fullscreen, popups, PiP).
    /// Use this for evaluation that merely observes the page. Main frame,
