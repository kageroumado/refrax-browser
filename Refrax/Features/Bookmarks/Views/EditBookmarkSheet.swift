...
/// Sheet for editing an existing bookmark with full configuration options.
///
/// Provides fields for:
/// - URL (required)
/// - Title
...
struct EditBookmarkSheet: View {
    ...
    private func loadBookmarkData() {
        ...
        selectedFolderID = bookmark.folderID
        tags = bookmark.tags
        selectedSpaceID = bookmark.spaceID
        isFavorite = bookmark.isFavorite
        favoriteMode = bookmark.favoriteMode
    }
    ...
    private func saveChanges() {
        ...
        bookmarksManager.updateBookmark(
            bookmark,
            title: title.isEmpty ? bookmark.url.host ?? "Untitled" : title,
            url: url,
            tags: tags,
            folder: folder,
        )
        ...
        bookmark.spaceID = selectedSpaceID
        
        // Update favorite status
        if isFavorite != bookmark.isFavorite {
            if isFavorite {
                bookmarksManager.addToFavorites(bookmark, mode: favoriteMode)
            } else {
                bookmarksManager.removeFromFavorites(bookmark)
            }
        } else if isFavorite, favoriteMode != bookmark.favoriteMode {
            bookmarksManager.changeFavoriteMode(bookmark, to: favoriteMode)
        }
...
#Preview(traits: .modifier(RefraxPreviewModifier())) {
    // Preview requires a real bookmark
    Text("Edit Bookmark Sheet")
}
