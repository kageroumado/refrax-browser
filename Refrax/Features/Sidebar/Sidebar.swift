...
struct Sidebar: View {
  ...
  var body: some View {
    ...
    .alert(deleteAlertTitle, isPresented: $showDeleteConfirmation, presenting: spaceToDelete) {
      space in
      ...
      if tabManager.hasMultipleSpaces {
        Text(
          "Are you sure you want to delete '\(space.name)'? This will close all \(browserState.tabs(in: space).count) tabs in this space."
        )
      } else {
        Text(
          "This is your last space. All tabs will be closed and the space will be reset to default."
        )
      }
    }
  }
  ...
  private func dropZoneOverlay(
    ...
    let additionalOffset: CGFloat =
      if case .pinned = type {
        dragCoordinator.shouldShowFavoritesDropZone
          ? DropZoneLayout.favoritesDropZoneTotalHeight : 0
      } else { 0 }
  ...
  private var bottomControls: some View {
    VStack(spacing: Constants.Spacing.small) {
      ...
      // Main controls row, three glass platters
  ...
  private func rebuildLayoutAnimated() {
    withAnimation(
      .spring(
        response: Constants.Animation.springDuration,
  ...
  private func updateActiveSpace(to newSpaceID: UUID) {
    ...
    guard let targetSpace = spaceManager.spaces.first(where: { $0.id == newSpaceID }) else {
      return
    }

    let spaceLockManager = browserState.spaceLockManager
    let needsAuth =
      targetSpace.isLockEnabled && !spaceLockManager.unlockedSpaceIDs.contains(targetSpace.id)
  ...
  private func performSpaceSwitch(to newSpaceID: UUID) {
    let oldIndex =
      spaceManager.spaces.firstIndex(where: { $0.id == windowState.activeSpaceID }) ?? 0
...
extension View {
  /// Wraps a bottom-bar control group in the sidebar's glass platter.
  fileprivate func bottomPlatter() -> some View {
